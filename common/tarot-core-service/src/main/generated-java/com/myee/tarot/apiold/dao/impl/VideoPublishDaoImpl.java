...
public class VideoPublishDaoImpl extends GenericEntityDaoImpl<Long, VideoPublish> implements VideoPublishDao {
    ...
    public List<VideoPublish> listByStoreTime(Long storeId, Date now){
        ...
        query.where((qVideoPublish.videoBusiness.type.eq(Constants.API_OLD_TYPE_MUYE)).and(qVideoPublish.active.eq(true)))
                .orderBy(qVideoPublish.id.desc())
                .offset(0).limit(Constants.VIDEO_PUBLISH_MAX);
